...
if [ -d "/app/db/migrations/${ENV}" ]; then
  # Loop through all SQL files in the migrations directory in alphabetical order
  for migration in $(find /app/db/migrations/${ENV} -name "*.sql" | sort); do
    echo "Applying migration: $(basename $migration)"
    PGPASSWORD=$POSTGRES_PASSWORD psql -h localhost -U $POSTGRES_USER -d $POSTGRES_DB -v POSTGRES_SCHEMA=$POSTGRES_SCHEMA -f $migration
  done
else
